...
\subsection{Future Work (M)}
Based on the findings and limitations, several directions for future work can be identified. First, domain-specific data augmentation techniques could be developed to better match outdoor environment characteristics. This could include augmentation with realistic outdoor backgrounds, varied lighting conditions (e.g., direct sunlight, shadows), and perspective transformations that reflect typical camera angles for outdoor photography. Another approach would be the implementation of neural augmentation, which adapts the augmentation in respect to the improvement of the classifier using a neuronal network \citep{perez2017}.

Second, interpretability methods such as Grad-CAM could be applied to visualize which image regions contribute to classification decisions. This would indicate whether the model uses discriminative features (e.g., object shape, texture) or background information \citep{selvaraju2016}. Such analysis could inform refinements to model architectures and training strategies.

Third, alternative transfer learning strategies could be explored. Fine-tuning deeper layers of ResNet18 or applying other pretrained architectures such as EfficientNet or Vision Transformers may improve performance. Domain adaptation techniques that reduce the distribution shift between source (ImageNet) and target (outdoor) domains could also be investigated.

Finally, the outdoor test set could be expanded to include more environments, lighting conditions, and object instances. This would enable more detailed analysis of how environmental factors affect classification performance and inform the design of more robust models.
